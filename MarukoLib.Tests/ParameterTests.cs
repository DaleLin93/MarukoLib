...
using MarukoLib.Parametrization;
...
namespace SharpBCI.Tests
{
    ...
    public class ParameterTests
    {
        ...
        public void TestPresentConvert()
        {
            ...
            var typeConverter1 = TypeConverter<NodeType, string>.Of(GetName, ParseName);

            string GetNumStr(NodeType type) => ((int)type).ToString();
            NodeType ParseNumStr(string value) => (NodeType) int.Parse(value);
            var typeConverter2 = TypeConverter<NodeType, string>.Of(GetNumStr, ParseNumStr);
