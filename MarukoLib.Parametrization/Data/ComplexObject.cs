...
namespace MarukoLib.Parametrization.Data
{
    ...
    public struct ComplexObject : IParameterizedObject
    {
        ...
        public ComplexObject(IParameterDescriptor[] keys, object[] values)
        {
            ...
            var pairs = new KeyValuePair<IParameterDescriptor, object>[values.Length];
            for (var i = 0; i < values.Length; i++)
                pairs[i] = new KeyValuePair<IParameterDescriptor, object>(i < 0 || i >= keys.Length ? null : keys[i], values[i]);
            _pairs = pairs;
        }
